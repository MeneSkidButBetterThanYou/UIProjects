...
vape:Clean(lplr.OnTeleport:Connect(function(teleportState)
    if teleportState == Enum.TeleportState.Started then
        if (not teleportedServers) and (not shared.VapeIndependent) then
            ...
            local teleportScript = [[
                ...
                shared.VapeCustomProfile = "myconfig"
            ]]
...
-- Load configuration
-- ensure a default profile is always defined before loading
shared.VapeCustomProfile = shared.VapeCustomProfile or "myconfig"

spawn(function()
if shared.vapereload then
    shared.vapereload = nil
    vape:Load()
elseif not vape.Loaded then
    vape:Load()
end end)

-- Robust self-queue debugger (fixed: avoid string.format on untrusted data)
-- Paste into your runner. It will print diagnostics to console.

-- === CONFIG: edit these to match your state/toggles ===
local myState = {
...
-- Local apply (current run). Replace body with your real toggle calls.
local function applyState_local(state)
    print("[applyState_local] applying state:", state and state.profileName)
    if not state then return end
    for k,v in pairs(state.toggles or {}) do
        print(("[applyState_local] toggle %s = %s"):format(tostring(k), tostring(v)))
        -- e.g. MyModule.Toggle(k, v)
    end
    if state.numbers and state.numbers.sens then
        print("[applyState_local] sensitivity =", state.numbers.sens)
    end
end

applyState_local(myState)

-- === safe serializer (table -> literal) ===
local function serialize(v, seen)
    ...
    if t == "table" then
        ...
        local parts = {}
        if isArray then
            for i=1,max do parts[#parts+1] = serialize(v[i], seen) end
            ...
            for k,val in pairs(v) do
                ...
                parts[#parts+1] = key.."="..serialize(val, seen)
            end
            return "{"..table.concat(parts, ",").."}"
        end
    end
    return string.format("%q", "<unsupported:"..t..">")
end

-- === try many queue_on_teleport providers and return one that works ===
local function find_queue_function()
    local candidates = {}

    if type(queue_on_teleport) == "function" then candidates[#candidates+1] = queue_on_teleport end
    if type(_G.queue_on_teleport) == "function" then candidates[#candidates+1] = _G.queue_on_teleport end
    if syn and type(syn.queue_on_teleport) == "function" then candidates[#candidates+1] = syn.queue_on_teleport end
    if fluxus and type(fluxus.queue_on_teleport) == "function" then candidates[#candidates+1] = fluxus.queue_on_teleport end
    if KRNL_LOADED and type(queue_on_teleport) == "function" then candidates[#candidates+1] = queue_on_teleport end

    -- also try scanning globals for functions containing "queue" and "teleport"
    for k,v in pairs(_G) do
        if type(k) == "string" and type(v) == "function" then
            local kn = k:lower()
            if kn:find("queue") and kn:find("teleport") then
    ...
    for i,fn in ipairs(candidates) do
        local ok, err = pcall(function()
            -- pass a tiny no-op string; if executor accepts it we assume it's valid
            fn("-- test payload (no-op)\n")
        end)
        if ok then
            return fn, i
        end
...
-- === build queued payload WITHOUT using string.format on untrusted data ===
local function build_payload(state)
    local literal = serialize(state) or "{}"
    -- Use concatenation so any '%' inside strings won't be interpreted
    local payload = "repeat task.wait() until game:IsLoaded()\n" ..
    "-- queued payload: reapply state (will print confirmation)\n" ..
    "local state = " .. literal .. "\n" ..
    "local function applyStateQueued(s)\n" ..
    "    pcall(function()\n" ..
    "        print('[queued] reapplying profile:', s and s.profileName)\n" ..
    "        for k,v in pairs(s.toggles or {}) do\n" ..
    "            print((' [queued] toggle %s = %s'):format(tostring(k), tostring(v)))\n" ..
    "        end\n" ..
    "        if s.numbers and s.numbers.sens then print('[queued] sens', s.numbers.sens) end\n" ..
    "    end)\n" ..
    "end\n" ..
    "applyStateQueued(state)\n"
    return payload
end

-- === attempt to queue payload ===
local qfn, idx = find_queue_function()
if not qfn then
    warn("[bootstrap] no queue_on_teleport detected. Executor may not support remote queuing.")
    print("[diagnostic] Look for APIs: queue_on_teleport, syn.queue_on_teleport, fluxus.queue_on_teleport")
    return
end

print(("[diagnostic] selected queue function candidate index: %s (pcall succeeded)"):format(tostring(idx)))

local payload = build_payload(myState)
local ok, err = pcall(function() qfn(payload) end)
if ok then
    print("[diagnostic] payload successfully queued. You should see [queued] messages in the next server once it runs.")
else
    warn("[diagnostic] failed to queue payload. pcall returned:", err)
    print("[diagnostic] payload length:", #payload)
end

-- expose helper to re-queue on demand
_G.DEBUG_QUEUE_PAYLOAD = function()
    local ok2, err2 = pcall(function() qfn(build_payload(myState)) end)
    print("requeue attempt:", ok2, err2)
end
...
local Functions = {
    Part = function()
        ...
        local parts = workspace:GetPartBoundsInBox(
            rootPart.CFrame + Vector3.new(0, 1, 0), 
            rootPart.Size + Vector3.new(1, entitylib.character.HipHeight or 2, 1), 
            overlapCheck
        )
        ...
        -- Fixed: Use a different approach instead of table.find
        local partsLookup = {}
        for _, part in pairs(parts) do
            partsLookup[part] = true
        end
        
        for part, _ in pairs(modified) do
            if not partsLookup[part] then
                modified[part] = nil
...
local OUTFITS = {
    ...
    vape.Categories.Render:CreateModule({
        ...
        Default = true,
        Function = function(isEnabled)  -- ✅ isEnabled is boolean (true/false)
            local Players = game:GetService("Players")
            local RunService = game:GetService("RunService")
            local LocalPlayer = Players.LocalPlayer
            ...
            local connections = {}
            local teamMonitorRunning = false
            ...
            local function setupTeamChangeMonitor()
                teamMonitorRunning = true
                coroutine.wrap(function()
                    local lastTeam = nil
                    while teamMonitorRunning and isEnabled do  -- ✅ Check both flags
            ...
            local function init()
                ...
                table.insert(connections, CharacterContainer.ChildAdded:Connect(function(child)
                ...
                if LocalPlayer:FindFirstChild("SelectedTeam") then
                    table.insert(connections, LocalPlayer.SelectedTeam:GetPropertyChangedSignal("Value"):Connect(function()
                        if isEnabled then
                            createFullOutfit()
                        end
                    end))
...
run(function()
    ...
    local function wipe(status)
        for _, inst in ipairs(status:GetChildren()) do
            if BLOCKED_NAMES[inst.Name] then
                pcall(function()
                    inst:Destroy()
                end)
            end
    ...
    local function attachBypass()
        ...
        local status = char:WaitForChild("Status", 5) -- Reduced from 15-10 to 5 seconds
        ...
        if loopThread then
            pcall(function() loopThread:Disconnect() end)
        end
        
        -- Set up new connections
        statusConn = status.ChildAdded:Connect(function(child)
            if BLOCKED_NAMES[child.Name] then
                task.defer(function()
                    pcall(function()
                        child:Destroy()
                    end)
        ...
        loopThread = RunService.RenderStepped:Connect(function()
            wipe(status)
        end)
    end
    ...
    vape.Categories.Blatant:CreateModule({
        Name = "StunDisabler",
        Tooltip = "Removes Stun for Headers, Bikes blah blah blah!",
        Default = false,
...
run(function()
    ...
    local BASE_BALL_SIZE = 1.6  -- The actual ball size in the game
    ...
    local function disableAnticheat()
        ...
        local remoteName = "touchdelaybenchmark"
    ...
    local function applySizeAndSpoof(val)
        ...
        -- Set the actual hitbox size (this is what affects collision)
        ball.Size = Vector3.new(val, val, val)
        
        if not fakeMesh or fakeMesh.Parent ~= ball then
            local existing = ball:FindFirstChild("VisualSizeSpoofer")
            if existing then
                existing:Destroy()
            end
            ...
            fakeMesh.Parent = ball
        end
        
        -- Fix: Calculate the correct scale to maintain visual size
        -- If we want the ball to appear normal size (1.6) but have larger hitbox (val)
        -- Scale = desired_visual_size / actual_size
        local spoofScale = BASE_BALL_SIZE / val
        fakeMesh.Scale = Vector3.new(spoofScale, spoofScale, spoofScale)
    end

    Module = vape.Categories.Blatant:CreateModule({
        Name = "HitboxExtender",
        Tooltip = "Have fun getting votekicked lol",
        Default = false,
        Function = function(enabled)
            local ball = workspace:FindFirstChild("Temp") and workspace.Temp:FindFirstChild("Ball")
            if not ball or not ball:IsA("BasePart") then
            ...
            if enabled then
                ...
                ball.Size = Vector3.new(BASE_BALL_SIZE, BASE_BALL_SIZE, BASE_BALL_SIZE)
                local mesh = ball:FindFirstChild("VisualSizeSpoofer")
                if mesh then
                    mesh:Destroy()
                end
                fakeMesh = nil
    ...
    SizeSlider = Module:CreateSlider({
        Name = "Size",
        Min = 1,
        Max = 30,
        Default = 5,
			...
			if callback then
				...
				heartbeatConnection = RunService.Heartbeat:Connect(function()
					...
					if trapZoneMarker then
						...
						trapZoneMarker.Transparency = 0.7
